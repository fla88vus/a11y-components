...
import { combineClassNames, isButtonDisabled, getLoadingLabel } from './Button.utils';

describe('Button Utils - Unit Tests', () => {
  describe('combineClassNames', () => {
    it('combines multiple class names', () => {
      expect(combineClassNames('btn', 'primary', 'large')).toBe('btn primary large');
    });

    it('filters out falsy values', () => {
      expect(combineClassNames('btn', false, 'primary', undefined, null)).toBe('btn primary');
    });
    ...
    it('handles conditional classes', () => {
      ...
      expect(combineClassNames('btn', isActive && 'active', isDisabled && 'disabled')).toBe(
        'btn active'
      );
    });
  });
