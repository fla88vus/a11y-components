...
describe("Radio - Accessibility Tests (WCAG 2.1 AA)", () => {
  ...
  describe("Automated Accessibility Testing", () => {
    ...
    it("has no violations when disabled", async () => {
      const { container } = render(
        <Radio name="plan" value="basic" label="Basic Plan" disabled />
      );
      const results = await axe(container);
      expect(results).toHaveNoViolations();
    });
    ...
    it("has no violations when required", async () => {
      const { container } = render(
        <Radio name="plan" value="basic" label="Basic Plan" required />
      );
      const results = await axe(container);
      expect(results).toHaveNoViolations();
    });
    ...
    it("meets 2.4.8 Location - label provides context", () => {
      render(
        <Radio
          name="billing"
          value="monthly"
          label="Monthly Subscription - $9.99/month"
        />
      );
      const radio = screen.getByRole("radio", {
    ...
    it("announces label correctly", () => {
      render(
        <Radio
          name="plan"
          value="pro"
          label="Pro Plan - Best Value - $19/month"
        />
      );
      const radio = screen.getByRole("radio", {
