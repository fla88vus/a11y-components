...
describe('Input', () => {
  ...
  it('hides helper text when error is present', () => {
    render(<Input label="Email" helperText="We'll never share your email" error="Invalid email" />);
  ...
  describe('Accessibility - WCAG 2.1 AA/AAA', () => {
    ...
    it('uses aria-describedby for helper text (WCAG 3.3.2)', () => {
      ...
      expect(document.getElementById(helperId!)).toHaveTextContent('Must be 8+ characters');
    });

    it('combines error and helper text in aria-describedby', () => {
      const { rerender } = render(<Input label="Email" helperText="Required field" />);

      let input = screen.getByLabelText('Email');
      const helperTextId = input.getAttribute('aria-describedby');

      rerender(<Input label="Email" helperText="Required field" error="Invalid email" />);
