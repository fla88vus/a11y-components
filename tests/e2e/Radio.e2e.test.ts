...
test.describe("Radio - E2E Tests", () => {
  ...
  test("radio buttons in same group are mutually exclusive", async ({
    page,
  }) => {
    await page.goto(`${STORYBOOK_URL}/iframe.html?id=atoms-radio--radio-group`);
  ...
  test("once checked, cannot be unchecked by clicking again", async ({
    page,
  }) => {
    await page.goto(`${STORYBOOK_URL}/iframe.html?id=atoms-radio--default`);
  ...
  test("shows focus indicator when focused", async ({ page }) => {
    ...
    // Deve avere un outline visibile (non 'none')
  ...
  test("has aria-describedby for helper text", async ({ page }) => {
    await page.goto(
      `${STORYBOOK_URL}/iframe.html?id=atoms-radio--with-helper-text`
    );
